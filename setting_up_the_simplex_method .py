...
# 設置變量數量和約束數量
lp.num_col_ = 2  # x1, x2
lp.num_row_ = 3  # 三個約束: x1<=4, 2x2<=12, 3x1+2x2<=18
...
# 設置約束矩陣（CSR格式）
lp.a_matrix_.start_ = np.array([0, 1, 3])  # 每個變量的非零係數起始位置
lp.a_matrix_.index_ = np.array([0, 1, 2])  # 非零係數所在的行索引
lp.a_matrix_.value_ = np.array([1, 2, 2], dtype=np.double)  # 非零係數值
